...
describe('graph component points', () => {
    describe('getCurrentWaterLevels', () => {
        ...
        it('returns correct object', () => {
            const retVal = getCurrentWaterLevels(null).resultFunc(mockLevels, 'level1');
            expect(retVal).toBe('level1 obj');
        });

        it('returns empty object when key does not exist', () => {
            const retVal = getCurrentWaterLevels(null).resultFunc(mockLevels, 'level2');
    ...
    describe('getCurrentWaterLevelUnit', () => {
        it('first unit in water level data', () => {
            expect(getCurrentWaterLevelUnit(null).resultFunc({
                samples: [{
                    unit: 'feet'
                }, {
                    unit: 'meters'
        ...
        it('returns null on non-existent dataset', () => {
            expect(getCurrentWaterLevelUnit(null).resultFunc({})).toBe(null);
        });
    ...
    describe('getChartPoints', () => {
        it('transforms service data into chartable points', () => {
            expect(getChartPoints(null).resultFunc({
                samples: [{
    ...
    describe('getExtentX', () => {
        it('returns correct date range', () => {
            expect(getExtentX(null).resultFunc([{
                dateTime: new Date('2010-10-10')
            }, {
        ...
        it('returns [undefined, undefined] on empty range', () => {
            expect(getExtentX(null).resultFunc([])).toEqual([undefined, undefined]);
        });

        it('works with mock state', () => {
            const store = getMockStore();
            expect(getExtentX(23)(store.getState())).not.toBe(null);
        });
    });
    ...
    describe('getLineSegments', () => {
        it('should separate on approved', () => {
            ...
            const value = 1;
            expect(getLineSegments(null).resultFunc([
    ...
    describe('getActiveClasses', () => {
        it('returns approved when any point is approved', () => {
            expect(getActiveClasses(null).resultFunc([
                {class: 'approved'},
                {class: 'provisional'},
                {class: 'provisional'},
                {class: 'provisional'}
